...
export default function StatsCards() {
  ...
  const cards = [
    {
      title: "Total Data",
      value: stats.totalData?.toLocaleString() || "0",
  ...
  return (
    <div className="w-full grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 md:gap-8 mb-2">
      {cards.map((stat, index) => (
        <motion.div
          key={index}
          initial={{ opacity: 0, y: 22, scale: 0.99 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          transition={{ duration: 0.36 + index * 0.09, type: "spring" }}
          className={`
            rounded-2xl p-4 md:p-6 border
            group cursor-pointer select-none relative overflow-hidden
            hover:scale-[1.03] transition-all duration-200 min-w-0
            ${theme === "dark"
        ...
        >
          ...
          <span
            className={`absolute z-0 right-[-16%] bottom-[-30%] w-[110px] h-[110px] rounded-full blur-2xl opacity-40 pointer-events-none bg-gradient-to-br ${stat.color}`}
          />
          <div className="flex items-center justify-between relative z-10">
            <div className="flex-1 min-w-0">
              <p className={`truncate text-sm font-medium ${theme === "dark" ? "text-white/70" : "text-gray-600"}`}>{stat.title}</p>
              <p className={`text-2xl md:text-3xl font-bold tracking-tight mt-2 ${theme === "dark" ? "text-white" : "text-gray-900"}`}>
                {stat.value}
              </p>
            </div>
            <div
